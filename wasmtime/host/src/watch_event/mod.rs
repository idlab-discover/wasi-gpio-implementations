    ...
    pub fn watch_event(
        ...
        let trigger = match map.get_mut(&key) {
            ...
            None => {
                ...
                let value = WatchEventValue { trigger: trigger.clone(), thread };

                // ADD TO MAP, WILL ALWAYS HAPPEN FIRST BECAUSE WE HAVE A LOCK ON THE MAP
                map.insert(key, value).expect("This should not happen");
...
pub trait WatchEventDigitalPin {
    fn is_low(&self) -> bool;
